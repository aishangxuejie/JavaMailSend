 ...
 *
 */
public class MysqlJdbcConnectionsPool implements DataSource {
	...
	private static String MYSQLDB_PROPERTIES = "\\resource\\mysqldb.properties";
	static {
		Properties properties = new Properties();
    	String realpath = System.getProperty("user.dir");
    	String path = realpath + MYSQLDB_PROPERTIES;
		...
		//取消此方式获取配置文件
//		ResourceBundle resource = ResourceBundle.getBundle("com/aishangxuejie/config/mysqldb");
...
//		JDBC_CONNECTION_InitSize = Integer.parseInt(resource.getString("jdbcConnectionInitSize"));
	...
	public Connection getConnection() throws SQLException {
		...
		if (linkedlist.size() > 0) {
			...
			System.out.println("MySQL--linkedlist数据库连接池大小" + linkedlist.size());
			/*
			 * 返回一个Connection对象，并且设置Connection对象方法调用的限制，
			 * 当调用connection类对象的close()方法时会将Connection对象重新收集放入linkedlist集合中。
			 */
			return (Connection) Proxy.newProxyInstance(MysqlJdbcConnectionsPool.class.getClassLoader(),
					conn1.getClass().getInterfaces(), new InvocationHandler() {//conn1.getClass().getInterfaces()此处与数据库驱动有关，oracle会有警告
						...
						public Object invoke(Object proxy, Method method, Object[] args) throws Throwable {
							if (!method.getName().equalsIgnoreCase("close")) {
								...
								System.out.println(conn1 + "对象被释放，重新放回MySQL--linkedlist集合中！");
								System.out.println("此时MySQL--Linkedlist集合中有" + linkedlist.size() + "个数据库连接对象！");
								return null;
							}
						}
					});
